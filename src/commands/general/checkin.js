...
module.exports = {
    data: new SlashCommandBuilder()
      ...
      async execute(interaction = CommandInteraction) {
        ...
        const embed = new StringSelectMenuBuilder()
        .setPlaceholder('請選擇')
        .setCustomId(`checkin_select_${interaction.user.id}`);
        ...
        for (let i = 0; i < checkin.length; i++) {
          if (checkin[i].guild == interaction.guildId) {
            embed.addOptions(
              ...
              .setLabel(checkin[i].checkincontent)
              .setValue(checkin[i].checkincontent)
              .setDescription(checkin[i].checkincontent)
            )
          }
        }
