...
// 設定 JSON 文件的路徑
const subscriptionsPath = "src/JSON/subscriptions.json";
const YOUTUBE_API_KEY = config.youtubeapikey;
const checkpath = "src/JSON/checkYTtemp.json";
...
function saveCheck(check) {
    try {
        ...
        fs.writeFileSync("src/JSON/checkYTtemp.json", datacheck);
    } catch (error) {
...
module.exports = {
    ...
    async execute(client) {
        ...
        client.user.setActivity(`Use /chat to converse with the most advanced AI.`);
